...
module.exports = {
	...
	async execute(reaction, user) {
		if (!user.bot) {
            ...
            db.all(`SELECT jobid FROM jobs JOIN events ON jobs.eventid = events.uuid JOIN announcements ON jobs.eventid = announcements.eventuuid WHERE messageid = ? AND role = ? AND userid = ?;`, [reaction.message.id, reaction.emoji.name, user.id], async (err, rows) => {
                rows.forEach(row => {
